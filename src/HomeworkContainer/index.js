...
class HomeworkContainer extends Component {
	...
	getItems = async () => {
	    ...
	    const assignmentsJson = await fetch('https://limitless-lowlands-56171.herokuapp.com/assignment', {
	      credentials: 'include'
	    })
	...
	addAssignment = async (name, link, notes) => {
		...
		const assignments = await fetch('https://limitless-lowlands-56171.herokuapp.com/assignment', {
			method: 'POST',
			body: JSON.stringify({
				name: name,
				link: link,
	...
	removeAssignment = async () => {
		...
		const removeItem = await fetch('https://limitless-lowlands-56171.herokuapp.com/assignment/' + id, {
			method: 'DELETE',
			credentials: 'include'
		});
	...
	editAssignment = async (name, link, notes) => {
		...
		const assignment = await fetch(`https://limitless-lowlands-56171.herokuapp.com/assignment/${editId}`, {
			method: 'PUT',
			body: JSON.stringify({
				name: name,
				link: link,
	...
	showHWModal = async (e)=>{
		...
		const hwJSON = await fetch(`https://limitless-lowlands-56171.herokuapp.com/assignment/${hwId}`, {
			credentials:'include'
		});
	...
	check = async (e) => {
		...
		const checkJSON = await fetch(`https://limitless-lowlands-56171.herokuapp.com/assignment/${this.state.hwShowing.id}/check`, {
			method: 'PUT',
			credentials: 'include'
		});
