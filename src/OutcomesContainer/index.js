...
class OutcomesContainer extends Component {
	...
	getItems = async () => {
	    const outcomesJson = await fetch('https://limitless-lowlands-56171.herokuapp.com/outcome', {
	      credentials: 'include'
	    })
	...
	openAdd = async (e) => {
  	...
  	addAssignment = async (name, link, notes) => {
		...
		const outcomes = await fetch('https://limitless-lowlands-56171.herokuapp.com/outcome', {
			method: 'POST',
			body: JSON.stringify({
				name: name,
				link: link,
	...
	removeAssignment = async () => {
		...
		const removeItem = await fetch('https://limitless-lowlands-56171.herokuapp.com/outcome/' + id, {
			method: 'DELETE',
			credentials: 'include'
		});
	...
	editAssignment = async (name, link, notes) => {
		...
		const outcome = await fetch(`https://limitless-lowlands-56171.herokuapp.com/outcome/${editId}`, {
			method: 'PUT',
			body: JSON.stringify({
				name: name,
				link: link,
	...
	showOutcomesModal = async (e)=>{
		...
		const outcomeJSON = await fetch(`https://limitless-lowlands-56171.herokuapp.com/outcome/${outcomeId}`, {
			credentials:'include'
		});
	...
	check = async (e) =>{
		...
		const checkJSON = await fetch(`https://limitless-lowlands-56171.herokuapp.com/outcome/${this.state.outcomeShowing.id}/check`, {
			method: 'PUT',
			credentials: 'include'
		});
